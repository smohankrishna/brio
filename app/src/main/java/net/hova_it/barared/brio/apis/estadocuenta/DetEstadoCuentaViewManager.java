 ...
 */
public class DetEstadoCuentaViewManager implements DetEstadoCuentaViewFragment.DetEstadoCuentaViewFragmentListener, BrioManagerInterface {
    private final String TAG = "BRIO_WEB";
    private String URL;

    private Context context;
    private FragmentManager fragmentManager;
    private DetEstadoCuentaViewFragment detEstadoCuentaViewFragment;
    ...
    private static DetEstadoCuentaViewManager mInstance = null;

    private BrioAndroidInterface brioAndroidInterface;

    public static DetEstadoCuentaViewManager getInstance(Context context) {
        if (mInstance == null) {
            mInstance = new DetEstadoCuentaViewManager(context, null);
        }
        return mInstance;
    }

    public static DetEstadoCuentaViewManager getInstance(Context context, Fragment parent) {
        if (mInstance == null) {
            mInstance = new DetEstadoCuentaViewManager(context, parent);
        }
        return mInstance;
    }

    private DetEstadoCuentaViewManager(Context context, Fragment parent) {
        this.context = context;
        this.parent = parent;
        this.detEstadoCuentaViewFragment = DetEstadoCuentaViewFragment.newInstance();
        this.modelManager = new ModelManager(context);
        this.sessionManager = SessionManager.getInstance(context);

        // INI: Agregado por: Manuel Delgadillo - 21/FEB/2017
        this.URL = "http://" + BrioGlobales.URL_WEB_BRIO + "?Modulo=Reportes&Seccion=DetCta"; // Sobre escribir la variable URL
    ...
    public void onFragmentCreated() {
        ...
        Map fields = detEstadoCuentaViewFragment.getFields();
    ...
    public Fragment createFragments(FragmentManager fragmentManager, int containerId) {
        ...
        detEstadoCuentaViewFragment.setEstadoCuentaViewManager(this);
    ...
    public void removeFragments() {
        ...
        this.fragmentManager
                .beginTransaction()
                .remove(detEstadoCuentaViewFragment)
                .commit();
    }
    ...
    final class BrioAndroidInterface {
        ...
        @JavascriptInterface
        public void verTicket(final int IdTicket){
            ((BrioActivityMain) context).runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    String superTicketHTML;
                    //Log.w("Reportes", "Banco adapter id ticket=" + IdTicket.ge ticketP.getIdTicket());
                    superTicketHTML = HTMLTicketPatcher.superTicketToHTML(IdTicket, context);
                    if (superTicketHTML == null){
    
                        BrioAlertDialog bad = new BrioAlertDialog((BrioActivityMain) context, context.getString(R.string.ticket_no_encontrado_titulo), context.getString(R.string.ticket_no_encontrado_mensaje));
                        bad.show();
                    }
                    else{
                        BrioWebViewDialog webViewDialog = new BrioWebViewDialog((BrioActivityMain) context, superTicketHTML);
                        //Log.w("Reportes", "Ventas enviando a imprimir=" + ticketP.getIdTicket());
                        webViewDialog.setIdTicket(IdTicket);
                        webViewDialog.show();
                    }
                }
            });
         ...
         */
        @JavascriptInterface
        public void mostrarTicket(final String Ticket){
            ((BrioActivityMain) context).runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    String superTicketHTML;
                    SuperTicket STicket;
                    try {
                        JSONObject JSTicket = new JSONObject(Ticket);
                        STicket = Utils.getSuperTicketFromJS(context, JSTicket);
                        superTicketHTML = HTMLTicketPatcher.superTicketToHTML(STicket,context);

                        BrioWebViewDialog BWD = new BrioWebViewDialog((BrioActivityMain) context,superTicketHTML);

                        BWD.setIdTicket(STicket.getTicket().getIdTicket());
                        BWD.show(STicket);
